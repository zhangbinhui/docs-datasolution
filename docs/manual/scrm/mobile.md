...
支持管理员在知客后台的【配置管理】中，通过参数设置用户免密登录的有效期。业务用户登录系统后，在此时间段内保持免密登录状态。超过此时长后，登录状态将失效，用户需要重新输入账号密码进行登录。

有效期的格式为（以下举例说明）：0.5H（半小时）、12H（12小时）、7D（7天）

参数名称“pwdEffectiveTime”。

![descript](./mobile-media/image8-6-1.png)
