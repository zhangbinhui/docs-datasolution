...
4)如需切换账号，点击【我的】->【设置】，在屏幕下方可看到切换账号功能；

<img src="./mobile-media/image6.png" alt="描述" width="300" />

<img src="./mobile-media/image7.png" alt="描述" width="300" />

5)若之前绑定了微信号，则需要先在【安全中心】中进行解绑，解绑后方可进行账号切换，如图：

<img src="./mobile-media/image8.png" alt="描述" width="300" />
...
前提条件：登录企业微信版知客，首先需要加入该企业微信群组（可主动扫描二维码加入，或者由企业微信管理员邀请加入）；
...
![descript](./mobile-media/image8-3.png)
...
3)登录成功后，点击下方【工作台】，即可使用知客企业微信版的功能，如图。

<img src="./mobile-media/image8-5.png" alt="描述" width="300" />

4)企业微信PC端，选择工作台也可使用到知客小程序功能，如图。

![descript](./mobile-media/image8-6.png)

5)免密登录有效期：

支持管理员在知客后台的【配置管理】中，通过参数设置用户免密登录的有效期。相关参数的设置和说明如下：

| 参数名称 | 说明 | 码值/格式 |
|---|---|---|
| pwdFreeLoginSwitch | **免密登录有效期开关。** 默认设置为0（关闭）。若将参数设置为1，表示开启，则会关联参数"pwdEffectiveTime"设置的免密登录有效期时间，进行免密登录，有效期失效后需要重新手动登录。知客默认免密登录，若无需有效期，则将参数设置为0。 | 1-开，0-关 |
| pwdEffectiveTime | **设置免密登录的有效期。** 当且仅当参数"pwdFreeLoginSwitch"设置为1时生效。用户登录系统后，在此时间段内保持免密登录状态。超过此时长后，登录状态将失效，用户需要重新输入账号密码或验证码进行登录。 | 格式（举例）：0.5H（半小时）、12H（12小时）、7D（7天） |
...
支持业务用户自定义设置需要订阅哪些类型的消息。

设置为"订阅"，消息推送到知客的同时也推送到业务用户的企微中。
...
![descript](./mobile-media/image37-1.png)

若某消息类型在后台的推送策略状态为“关闭”，订阅设置页面中展示“管理员未开启该类消息的推送”，表示该消息在系统中暂不支持推送，也就是说，即使业务用户订阅了该消息，系统也不会给业务用户推送该类消息，但用户的订阅设置会保存下来，待管理员在后台管理中将该消息的策略状态更改为“启用”后，用户即可收到该类消息的推送。

![descript](./mobile-media/image37-2.png)
...
实时展示品种持仓客户数量及客群列表。当特定品种出现重大行情或投资机会时，可以通过筛选品种查看实时持有该品种的客户群体。
...
![descript](./mobile-media/image41-3.png)

  - a) 潜客列表、潜客详情

<img src="./mobile-media/image41-1.png" alt="描述" width="300" />
<img src="./mobile-media/image41-2.png" alt="描述" width="300" />

  - b)跟进功能
  - c)消息模块的线索tab
  - d)重点客群、客户机会

若对接渠道1.0版本，跳转到的潜客列表如下图展示：

![descript](./mobile-media/image41.png)

若对接渠道2.0版本，跳转到的潜客列表如下图展示：

![descript](./mobile-media/image41-1.png)
...
潜在客户是客户增长的重点。若对接渠道1.0，则将通过二维码扫码留资的客户信息同步到知客潜客列表中。若对接渠道2.0，则通过渠道2.0系统获取渠道的线索和数据。未来将扩充其他渠道来源的潜客数据和潜客管理功能。

- 对接渠道1.0：

以客户留资的手机号作为唯一标识，潜客卡片会展示潜客姓名（如手机号为空，系统展示--）、手机号以及潜客触发的推荐标签。
...
以客户留资的手机号作为唯一标识，潜客卡片会展示手机号、微信昵称、渠道来源、跟进标签和最新留资时间,如图：
...
下图展示的是根据客户开户前后流程节点，统计的客户列表，列表表头也会根据当前分组主题进行适应调整；

![descript](./mobile-media/image50.png)

下图展示的是符合开通特殊品种交易权限条件的客户列表。

![descript](./mobile-media/image50-1.png)
...
<img src="./mobile-media/image67-2.png" alt="描述" width="300" />
...
<img src="./mobile-media/image67-3.png" alt="描述" width="300" />
<img src="./mobile-media/image67-4.png" alt="描述" width="300" />
...
- b)客户的持仓结构:按品种或按交易所统计持仓金额，手数，保证金占用等；

<img src="./mobile-media/image78.png" alt="描述" width="300" />
<img src="./mobile-media/image79.png" alt="描述" width="300" />

- c)客户的交易权限开通情况：展示客户已开通和未开通的特殊权限，以及客户的开通条件明细等；页面中通过颜色区分了客户交易权限的开通情况，如图：
  - 蓝色“已开通权限”标识客户已开通该交易权限。
  - 橙色“已符合开通”表示客户满足开通条件但暂未开通该交易权限，点击权限名称跳转到对应交易权限的开通条件明细页面。
  - 灰色“不符合开通”表示客户不满足通条件且暂未开通该交易权限点击权限名称跳转到对应交易权限的开通条件明细页面。

![descript](./mobile-media/image80.png)

:::tip
“资金要求”，若不满足“连续5个交易日”这一条件，则前端页面展示"--"。
:::

在页面上方点击“开通条件明细”，跳转查看当前客户“前连续5个交易日每日结算后保证金账户可用资金最小余额”、“近一年成功交易日明细”、“近三年内交易笔数”、适当性风险等级、证件信息和已开通交易所编码，如图：

![descript](./mobile-media/image80-1.png)

- d)客户的保证金率：可以同时查看该客户当前适用的保证金率和对应的保证金模板详情,客户当前的真实保证金率及相对于公司标准或交易所标准的浮动情况；

<img src="./mobile-media/image81.png" alt="描述" width="300" />

- e)客户的强平情况：统计出近期3个月的强平信息；

<img src="./mobile-media/image82.png" alt="描述" width="300" />

- f)客户交易信息：固定周期内的关键交易信息统计；
...
在【客户】模块中展示当前业务用户有权限看到的持仓客户数排名前5的品种，如图：

<img src="./mobile-media/image83-1.png" alt="描述" width="300" />

点击“更多”，跳转到“品种持仓客户列表”，展示所有品种的实时持仓客户数，列表支持按交易所和品种筛选。

点击列表中的品种名称，跳转至该品种的实时持仓客户列表，展示该品种的持仓客户持仓手数、多空占比和实时盈亏，如图：

<img src="./mobile-media/image83-2.png" alt="描述" width="300" />

## 五、【简报】功能介绍

围绕指标概览、开户专题、部门排行、活跃品种、次席客户、客户十大和员工业绩排行等主题，搭建起公司、部门、个人不同角色下的工作简报，支持日报和月报的切合，一手掌握工作进展，评估工作内容，及时调整业务方向。
...
1）点击该模块右上角的“更多”可以查看更多指标和数量的部门排行统计，如图：

![descript](./mobile-media/image96.png)

2）点击【部门榜单】页面中的“部门名称”，可下钻到该部门的下一层级部门，查看下一层级所有部门的排行。点击右上角的返回按钮，可返回到上一层，如图：

![descript](./mobile-media/image96-1.png)

3）点击【部门榜单】页面中指标列的位置，跳转到该部门的业绩明细页面，展示当前时间统计维度下该部门各项业绩指标的数据及同比、环比比值，如图：

![descript](./mobile-media/image96-2.png)

4）部门排行页默认同简报的统计周期一致，也支持灵活的切换和选择，如图：

![descript](./mobile-media/image96-3.png)

5）部门排行页面支持通过部门名称模糊查询并进行排序。同时支持筛选部门名称，查询所选中的部门及其子部门下所有部门的业绩排名，如图：

![descript](./mobile-media/image96-4.png)

6）为方便查看，支持对排行列表的表头进行排序或隐藏的设置。点击“更多设置”跳转到【榜单设置】页面，支持对字段设置“置顶”、“显示/隐藏”或通过拖拉“排序”列对表头字段进行排序，满足不同用户的个性需求，如图：

![descript](./mobile-media/image96-5.png)
...
根据管理者的角度对公司/部门的员工进行多维度的排行统计，仅“部门经理”或“总经理”角色用户查看的【部门简报】或【公司简报】展示该模块。

1）点击该模块右上角的“更多”可以查看更多指标和数量的员工业绩排行统计，如图：

![descript](./mobile-media/image100-1.png)

2）员工业绩排行页默认同简报的统计周期一致，也支持灵活的切换和选择，如图：

![descript](./mobile-media/image100-2.png)

3）员工业绩排行页面支持通过员工名称和代码模糊查询并进行排序。同时支持筛选部门名称，查询所选中的部门及其子部门下所有员工的业绩排名，如图：

![descript](./mobile-media/image100-3.png)

4）为方便查看，支持对排行列表的表头进行排序或隐藏的设置。点击“更多设置”跳转到【榜单设置】页面，支持对字段设置“置顶”、“显示/隐藏”或通过拖拉“排序”列对表头字段进行排序，满足不同用户的个性需求，如图：

![descript](./mobile-media/image100-4.png)

5）点击榜单页面中的行数据，跳转到该员工的业绩明细页面，展示当前时间统计维度下该员工各项业绩指标的数据及同比、环比比值，如图：

![descript](./mobile-media/image100-5.png)
...
![descript](./mobile-media/image103-1.png)
![descript](./mobile-media/image103-2.png)
...
【关于】页面增加了版本更新说明与操作说明入口，从1.9.2版本开始，业务用户都可以在这里了解到每个版本的更新概况，同时直接跳转到操作说明在线文档查看知客的功能使用说明。

<img src="./mobile-media/image122.png" alt="描述" width="300" />
<img src="./mobile-media/image123.png" alt="描述" width="300" />
